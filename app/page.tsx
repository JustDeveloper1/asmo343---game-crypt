...
export default function Page() {
    ...
    const [game, setGame] = useState<string>("fortnite");
    ...
    const handleInputChange = (event: ChangeEvent<HTMLTextAreaElement>) => {
        ...
        setInputText(text);
        if (mode === "encode") {
            setOutputText(encode(text, game));
        } else {
            setOutputText(decode(text, game));
        }
    };
    ...
    const handleModeChange = (value: string) => {
        setMode(value);
        if (value === "encode") {
            setOutputText(encode(inputText, game));
        } else {
            setOutputText(decode(inputText, game));
        }
    };
        ...
        <main className='bg-dark-200 w-full h-full flex flex-col'>
            ...
            <div className='flex-grow flex items-end justify-center p-4 rounded-lg'>
                <div className='cool-bg p-4 w-full max-w-4xl rounded-2xl border border-dark-300'>
                    ...
                    <GameSelector onGameChange={handleGameChange} />
